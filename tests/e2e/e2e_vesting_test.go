...
func (s *IntegrationTestSuite) testDelayedVestingAccount(api string) {
	...
	s.Run("test delayed vesting genesis account", func() {
		...
		balance, err := getSpecificBalance(api, vestingDelayedAcc.String(), akiiDenom)
		s.Require().NoError(err)
		s.Require().Equal(vestingBalance.AmountOf(akiiDenom), balance.Amount)
...
func (s *IntegrationTestSuite) testContinuousVestingAccount(api string) {
	s.Run("test continuous vesting genesis account", func() {
		...
		//	Check address balance
		balance, err := getSpecificBalance(api, continuousVestingAcc.String(), akiiDenom)
		s.Require().NoError(err)
		s.Require().Equal(vestingBalance.AmountOf(akiiDenom), balance.Amount)
...
func (s *IntegrationTestSuite) testPeriodicVestingAccount(api string) { //nolint:unused
	...
	s.Run("test periodic vesting genesis account", func() {
		...
		s.execCreatePeriodicVestingAccount(
			chain,
			periodicVestingAddr,
			filepath.Join(kiichainHomePath, vestingPeriodFile),
		...
		//	Check address balance
		balance, err := getSpecificBalance(api, periodicVestingAddr, akiiDenom)
		s.Require().NoError(err)

		expectedBalance := sdk.NewCoin(akiiDenom, math.NewInt(0))
		for _, period := range acc.VestingPeriods {
			// _, coin := ante.Find(period.Amount, akiiDenom)
			_, coin := period.Amount.Find(akiiDenom)
...
func generateVestingPeriod() ([]byte, error) {
	p := vestingPeriod{
		...
		Periods: []period{
			{
				Coins:  "850000000" + akiiDenom,
				Length: 35,
			},
			{
				Coins:  "2000000000" + akiiDenom,
				Length: 35,
			},
		},
	}
