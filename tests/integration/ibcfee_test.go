...
import (
	...
	kiichainApp "github.com/kiichain/kiichain/v2/app"
)
...
func (suite *IBCFeeTestSuite) SetupTest() {
	...
	suite.Require().True(ok, "expected App to be KiichainApp")
...
func getApp(chain *ibctesting.TestChain) *kiichainApp.KiichainApp {
	app, ok := chain.App.(*kiichainApp.KiichainApp)
	if !ok {
		panic("expected App to be KiichainApp")
	}
	return app
}
