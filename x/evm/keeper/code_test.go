...
func TestCode(t *testing.T) {
	...
	k.BankKeeper().MintCoins(ctx, "evm", sdk.NewCoins(sdk.NewCoin("ukii", sdk.OneInt())))
	k.BankKeeper().SendCoinsFromModuleToAccount(ctx, "evm", sdk.AccAddress(addr[:]), sdk.NewCoins(sdk.NewCoin("ukii", sdk.OneInt())))
	...
	require.Equal(t, sdk.AccAddress(addr[:]), k.AccountKeeper().GetAccount(ctx, k.GetKiiAddressOrDefault(ctx, addr)).GetAddress())
}
