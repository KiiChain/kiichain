...
CONFLICTING_DEFS=(
	"protobufAny" "runtimeError"
	"v1beta1Params" "v1beta1QueryParamsResponse"
	"v1beta1PageRequest" "v1QueryParamsResponse"
	"v1beta1Grant" "v1AccessType"
	"v1Params" "v1QueryCodeResponse"
	"v1beta1QueryDelegatorValidatorsResponse"
	"typesHeader" "v1Counterparty"
	"v1State"
)
...
for file in $(find "$SWAGGER_DIR" -name "*.swagger.json" | sort); do
  ...
  # Create a prefix from the module path (e.g., cosmos/bank/v1beta1 -> CosmosBankV1beta1)
