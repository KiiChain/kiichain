...
KIIVALOPER=$(printf "12345678\n" | "$HOME/go/bin/kiichaind" keys show $VALIDATOR_ACCOUNT --bech=val -a)
printf "12345678\n" | "$HOME/go/bin/kiichaind" tx oracle set-feeder "$ORACLE_ACCOUNT_ADDRESS" --from $VALIDATOR_ACCOUNT --fees 2000ukii -b block -y --chain-id kii >/dev/null 2>&1
...
sed -i.bak -e "s|^validator *=.*|validator = \"$KIIVALOPER\"|" $ORACLE_CONFIG_FILE
