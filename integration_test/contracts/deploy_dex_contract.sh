...
printf "12345678\n" | $seidbin tx dex register-contract "$contract_addr" "$contract_id" false true 100000000000 -y --from="$keyname" --chain-id="$chainid" --fees=100000000000ukii --gas=500000 --broadcast-mode=block
...
contract_pair=$(printf "12345678\n" | $seidbin tx dex register-pairs integration_test/contracts/"$contract_name"-pair.json -y --from=$keyname --chain-id=$chainid --fees=10000000ukii --gas=500000 --broadcast-mode=block --output=json)
