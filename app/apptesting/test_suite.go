...
import (
	...
	appparams "github.com/kiichain/kiichain/v7/app/params"
...
func (s *KeeperTestHelper) AllocateRewardsToValidator(valAddr sdk.ValAddress, rewardAmt math.Int) {
	...
	coins := sdk.Coins{sdk.NewCoin(appparams.DisplayDenom, rewardAmt)}
	err = banktestutil.FundModuleAccount(s.Ctx, s.App.BankKeeper, distrtypes.ModuleName, coins)
	s.Require().NoError(err)

	// allocate rewards to validator
	s.Ctx = s.Ctx.WithBlockHeight(s.Ctx.BlockHeight() + 1)
	decTokens := sdk.DecCoins{{Denom: appparams.DisplayDenom, Amount: math.LegacyNewDec(20000)}}
