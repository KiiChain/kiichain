...
func (app *App) LightInvarianceTotalSupply(cms sdk.CommitMultiStore) {
	defer metrics.MeasureSince(
		[]string{"kii", "lightinvariance_supply", "milliseconds"},
		time.Now().UTC(),
	)
	...
	for _, p := range balanceChangePairs {
		if len(p.Key) < 2 {
			// invalid key; ignore
			metrics.IncrCounterWithLabels([]string{"kii", "lightinvariance_supply", "invalid_changed_key"}, 1, []metrics.Label{
				{
					Name:  "type",
					Value: "kii",
				},
		...
		if !p.Delete {
			...
			if err := balance.Unmarshal(p.Value); err != nil {
				metrics.IncrCounterWithLabels([]string{"kii", "lightinvariance_supply", "unmarshal_failure"}, 1, []metrics.Label{
					{
						Name:  "type",
						Value: "ukii",
					}, {
