...
func (s *IntegrationTestSuite) writeUpdateRateLimitAtomProposal(c *chain) {
	template := `
	{
		...
		"metadata": "ipfs://CID",
		"deposit": "100akii",
		"title": "Update Rate Limit on (channel-0, akii)",
...
func (s *IntegrationTestSuite) writeResetRateLimitAtomProposal(c *chain) {
	template := `
	{
		...
		"metadata": "ipfs://CID",
		"deposit": "100akii",
		"title": "Reset Rate Limit on (channel-0, akii)",
...
func (s *IntegrationTestSuite) testAddRateLimits() {
	...
	s.T().Logf("Submitting, deposit and vote Gov Proposal: Add IBC rate limit for (channel-0, akii)")
	...
	s.Require().Eventually(
		func() bool {
			s.T().Logf("After AddRateLimit proposal (channel-0, akii)")
			...
			res, err := queryRateLimit(chainEndpoint, transferChannel, akiiDenom)
			s.Require().NoError(err)
	...
	s.Require().Eventually(
		func() bool {
			...
			s.Require().Equal(sdkmath.NewInt(10), rateLimits[1].Quota.MaxPercentSend)
...
func (s *IntegrationTestSuite) testResetRateLimit() {
	...
	s.T().Logf("Proposal number: %d", proposalCounter)
	s.T().Logf("Submitting, deposit and vote Gov Proposal: Reset IBC rate limit for (channel-0, akii)")
...
func (s *IntegrationTestSuite) testRemoveRateLimit() {
	...
	s.T().Logf("Proposal number: %d", proposalCounter)
	s.T().Logf("Submitting, deposit and vote Gov Proposal: Remove IBC rate limit for (channel-0, akii)")
...
func (s *IntegrationTestSuite) testIBCTransfer(expToFail bool) {
	...
	totalAmount, err := querySupplyOf(chainEndpoint, akiiDenom)
	s.Require().NoError(err)

	threshold := totalAmount.Amount.Mul(sdkmath.NewInt(1)).Quo(sdkmath.NewInt(100))
	tokenAmt := threshold.Add(sdkmath.NewInt(10)).String()
	s.sendIBC(s.chainA, 0, sender, recipient, tokenAmt+akiiDenom, standardFees.String(), "", expToFail)
