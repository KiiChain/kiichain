...
func (s *IntegrationTestSuite) testICARegisterAccountAndSendTx() {
	s.Run("register_ICA_account_and_send_tx_to_chainB", func() {
		...
		s.sendIBC(s.chainA, 0, icaOwnerAccount, icaAccount, strconv.Itoa(tokenAmount)+akiiDenom, standardFees.String(), "", false)
		...
		s.Require().Eventually(
			func() bool {
				icaAccountBalances, err = queryKiichainAllBalances(chainBAPIEndpoint, icaAccount)
		...
		s.Require().Eventually(
			func() bool {
				recipientBalances, err = queryKiichainAllBalances(chainBAPIEndpoint, recipientB)
				s.Require().NoError(err)
		...
		if recipientBalanceBefore.IsNil() {
			recipientBalanceBefore = math.ZeroInt()
		}
...
func (s *IntegrationTestSuite) registerICAAccount(c *chain, valIdx int, sender, connectionID, fees string) {
	...
	s.executeKiichainTxCommand(ctx, c, icaCmd, valIdx, s.defaultExecValidation(c, valIdx))
