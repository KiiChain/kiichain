...
NODE_HOME=$(realpath ./build/.kiichain)
echo "NODE_HOME = ${NODE_HOME}"
BINARY=$NODE_HOME/cosmovisor/genesis/bin/kiichaind
echo "BINARY = ${BINARY}"
CHAINID=cosmoshub-4
...
if ! test -f "./build/kiichaindold"; then
  echo "old kiichaind binary does not exist"
  exit
fi

rm -rf ./build/.kiichain

mkdir -p "$NODE_HOME"/cosmovisor/genesis/bin
cp ./build/kiichaindold "$NODE_HOME"/cosmovisor/genesis/bin/kiichaind
chmod a+x $BINARY
$BINARY init upgrader --chain-id $CHAINID --home "$NODE_HOME"

if ! test -f "./build/kiichaindnew"; then
  echo "new kiichaind binary does not exist"
  exit
fi

mkdir -p "$NODE_HOME"/cosmovisor/upgrades/$UPGRADE_VERSION/bin
cp ./build/kiichaindnew "$NODE_HOME"/cosmovisor/upgrades/$UPGRADE_VERSION/bin/kiichaind
chmod a+x "$NODE_HOME"/cosmovisor/upgrades/$UPGRADE_VERSION/bin/kiichaind
...
# replace default stake token with akii
sed -i -e '/total_liquid_staked_tokens/!s/stake/akii/g' $NODE_HOME/config/genesis.json
...
$BINARY genesis add-genesis-account val 10000000000000000000000000akii --home $NODE_HOME --keyring-backend test
$BINARY genesis gentx val 1000000000akii --home $NODE_HOME --chain-id $CHAINID
$BINARY genesis collect-gentxs --home $NODE_HOME

sed -i.bak'' 's/minimum-gas-prices = ""/minimum-gas-prices = "0akii"/' $NODE_HOME/config/app.toml
