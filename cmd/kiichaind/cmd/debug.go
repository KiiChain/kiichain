...
var modules = []string{
	"wasm", "aclaccesscontrol", "epoch", "mint", "acc", "bank", "crisis", "feegrant", "staking", "distribution", "slashing", "gov", "params", "ibc", "upgrade", "evidence", "transfer", "tokenfactory",
}
...
$ %s debug dump-iavl 12345
	...
	cmd.Flags().StringP(FlagDBPath, "d", "", "The path to the db, default is $HOME/.kiichain3/data/application.db")
...
func dumpIavlCmdHandler(cmd *cobra.Command, args []string) error {
	...
	if dbPath == "" {
		dbPath = fmt.Sprintf("%s/.kiichain3/data/application.db", home)
	}
