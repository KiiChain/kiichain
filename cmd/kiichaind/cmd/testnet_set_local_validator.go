...
func testnetUnsafeStartLocalValidatorCmd(ac appCreator) *cobra.Command {
	cmd := server.StartCmd(ac.newTestingApp, kiichain.DefaultNodeHome)
...
Example:
	simd testnet unsafe-start-local-validator --validator-operator="kiivaloper17fjdcqy7g80pn0seexcch5pg0dtvs45pah7f9m" --validator-pukey="SLpHEfzQHuuNO9J1BB/hXyiH6c1NmpoIVQ2pMWmyctE=" --validator-privkey="AiayvI2px5CZVl/uOGmacfFjcIBoyk3Oa2JPBO6zEcdIukcR/NAe64070nUEH+FfKIfpzU2amghVDakxabJy0Q==" --accounts-to-fund="kii1ju6tlfclulxumtt2kglvnxduj5d93a646lzg7x,kii1r5v5srda7xfth3hn2s26txvrcrntldjuzqa3pq" [other_server_start_flags]
	`
	cmd.Flags().String(flagValidatorOperatorAddress, "", "Validator operator address e.g. kiivaloper17fjdcqy7g80pn0seexcch5pg0dtvs45pah7f9m")
...
// returns kiichain app with modified application and consensus states by replacing validator related data
func (a appCreator) newTestingApp(
	...
	kiichainApp, ok := app.(*kiichain.KiichainApp)
	if !ok {
		panic(errors.New("invalid kiichain application"))
	}
	...
	// Update app state
	err = updateApplicationState(kiichainApp, args)
	if err != nil {
		panic(err)
	}

	// Update consensus state
	err = updateConsensusState(logger, appOpts, kiichainApp.CommitMultiStore().LatestVersion(), args)
	if err != nil {
		panic(err)
	}
...
func updateApplicationState(app *kiichain.KiichainApp, args valArgs) error {
