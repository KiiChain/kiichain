...
The association between the kiichain address and the eth address will also be created here if using keyring-backend test.
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if keyringBackend == keyring.BackendTest {
				...
				evmGenState.AddressAssociations = append(evmGenState.AddressAssociations, &kiichainEthAddrAssociation)
...
func getPrivateKeyOfAddr(kb keyring.Keyring, addr sdk.Address) (*ecdsa.PrivateKey, error) {
	...
	for _, key := range keys {
		...
		if localInfo.GetAddress().Equals(addr) {
			...
			// Need to use private key to convert to kiichain address here
